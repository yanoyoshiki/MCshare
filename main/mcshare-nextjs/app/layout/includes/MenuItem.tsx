...
export default function MenuItem({ iconString, colorString, sizeString }: MenuItemTypes) {
    ...
    const icons = () => {
        ...
        if (iconString == 'PIM Colloseum LIVE') return <BsCameraVideo size={sizeString} color={colorString} />
        if (iconString == 'Monthly SSG MCs') return <MdWorkspacePremium size={sizeString} color={colorString} />
    }
